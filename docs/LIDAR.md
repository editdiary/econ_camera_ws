...
**LiDAR만 bag 녹화** (카메라 없이 라이다만 수집·점검):
```bash
ros2 launch econ_camera_ros record_lidar.launch.py
```
LiDAR 노드 + record(mcap)만 뜬다. 녹화 토픽 4개: `/unilidar/cloud`, `/unilidar/imu`, `/tf`, `/tf_static`.

> LiDAR만 **발행만**(녹화 없이) 확인하려면 `ros2 launch unitree_lidar_ros2 launch_no_rviz.py`.
> **`ros2 run unitree_lidar_ros2 ...` 단독 실행 금지** — 노드 하드코딩 기본값의 `lidar_ip`/`local_ip`가 뒤바뀌어 있음. 반드시 launch로 실행.
> `record_lidar`와 `record_all`을 **동시에 실행하면** LiDAR 노드가 겹쳐 포트/장치 선점 충돌이 난다([`TROUBLESHOOTING.md`](TROUBLESHOOTING.md) T1).
...
> **점검 도구**: 카메라 녹화 품질은 `python3 tools/check_recording.py <bag>`(프레임 간격 판정),
> LiDAR 점군은 `python3 mapping/check_lidar_bag.py <bag>`(프레임수·rate·빈프레임 + BEV PNG)로
> `ros2 topic hz`보다 정밀하게 사후 판정할 수 있다.
