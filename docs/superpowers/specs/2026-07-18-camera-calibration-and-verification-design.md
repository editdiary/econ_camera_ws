...
[수집]  기존 capture 파이프라인으로 캘리브 시퀀스 녹화 (mcap, CompressedImage 4토픽, 하드웨어 동기)
   ...
   │
[브릿지]  추출 JPEG → Kalibr 입력 형식 변환   ← 신규 작성 (작은 스크립트)
   │        (ROS1 bag 또는 Kalibr가 읽는 이미지 폴더 형식)
   │
[Kalibr]  Docker 컨테이너에서 offline 실행 → intrinsic/extrinsic + 리포트(PDF)
```

- **신규 작업물**: 추출 JPEG → Kalibr 입력 브릿지. 나머지(capture, bag_extract)는 재사용.
- Kalibr는 **원본(비압축) 이미지**를 다룬다. JPEG 디코드는 브릿지/추출 단계에서 처리.
...
- Kalibr용 `aprilgrid.yaml`(태그 행·열, tagSize, tagSpacing)을 실측값으로 작성해 둔다.
...
1. **JPEG → Kalibr 입력 브릿지** — 추출 동기 세트를 Kalibr가 읽는 형식으로 변환(+ ~4Hz 다운샘플).
2. **현장 커버리지 체크 스크립트** — 검출률 + 커버리지 히트맵, `bag_extract` 출력 재사용.
3. **Kalibr Docker 실행 절차** — DS/eUCM 2회 실행 + 리포트 비교 워크플로우.
4. **검증 스크립트/절차** — cross-projection·루프 일관성·스티칭 육안 확인.
5. **`calib.yaml` 스키마** — 모델·파라미터·extrinsic·검증 결과 기록.
...
- AprilGrid 실측 파라미터(tagSize/tagSpacing) 확정.
