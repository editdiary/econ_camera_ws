...
사용자가 2026-08-10 에 제작 완료. `raws2/frame_000020` 기준으로 그렸고 클래스 3개로 나뉜다.

| 항목 | 값 |
|---|---|
| 파일 | `mask_front.png`, `mask_right.png`, `mask_left.png` + `labelmap.txt` (cam2=rear 는 미사용) |
| 위치 | `data/calib_260723/self_mask/` (`--self-mask-dir` 로 변경 가능) |
| 크기 | 1280×720 (원본 해상도) |
| 값 | 클래스 색 PNG. `background 0,0,0` = 사용 가능, 그 외 전부 무효 |
| 클래스 | `table 250,50,83` · `handle 61,245,61` · `human 140,120,240` (RGB) |

**기본으로 읽는 클래스는 `table` 만이다**(`--self-mask-classes`). `handle`·`human` 은 칠해져
있지만 쓰지 않는다 — 이미지에서의 위치가 프레임마다 달라 정적 마스크로는 못 맞히므로 §2.8.1 의
body 프레임 self 박스가 대신 덮는다. 마스크 파일에는 남겨 둔다: 셋 다 필요해지는 상황이 오면
옵션만 바꾸면 되고, 어느 클래스가 어디였는지가 기록으로 남는다.
| 안 칠해도 됨 | 어안 유효원 바깥 검은 영역 (자동 검출) |

실측 면적: front 5.21%(table only) / right 12.97%(table 6.97·human 4.77·handle 1.23) /
left 9.12%(table 2.85·human 5.44·handle 0.83). 고유색이 2~4개뿐이라 안티에일리어싱이 없다.

**table 이 상단·하단에 두 군데 나타난다.** 리그 구조 때문이다 — 카트에 간이 테이블을 붙이고
그 위에 별도 Rig 로 카메라를 배치한 뒤, 카메라 위에 작은 판(같은 연갈색)을 놓고 LiDAR 를
올렸다. 그 받침판이 일부 렌즈의 위쪽을 가린다. 지면 셀은 아래로 투영되므로 상단 가림은
BEV 판정에 거의 영향이 없지만 포함해도 손해가 없다.

**세 클래스를 전부 무효로 친다.** `human` 은 움직이는 사람의 정적 외피라 사람이 없는
프레임에서는 과하게 가리지만, 과하게 가리는 쪽은 안전하고(visibility=0 → loss 제외)
덜 가리는 쪽은 틀린 라벨이 된다.

**마스크는 7개 bag 전부에 그대로 쓴다.** 리그가 옮겨졌는지 검증했다 — table 마스크 안쪽
화소의 색이 7개 bag 전부에서 일관된 연갈색(R>G>B, 대략 150~210·125~190·90~155)이다.
리그가 움직였다면 어느 bag 에선 그 자리에 잎이나 바닥이 찍혔을 텐데 그런 bag 이 없다.

마스크가 없어도 파이프라인은 돈다. 그 경우 **근거리 visibility 가 낙관적**이라는 경고를 로그에
찍고 `meta.json` 에 `self_mask: null` 을 기록한다. 마스크가 준비되면 라벨만 재생성한다.

### 2.8.1 손잡이·수집자는 body 프레임 self 박스로 덮는다 (이미지 마스크가 아니라)

`−far <= x <= −near` 이고 `|y| <= yh` 인 셀은 `visibility=0` 이다.
**기본 `near=0.4 far=2.1 yh=0.7` → 896셀(6.2%).** CLI 옵션이다.

**왜 이미지 마스크가 아닌가.** 카트 손잡이와 수집자도 이미지에 찍히므로 마스크에 칠할 수는
있다. 그런데 그 위치가 프레임마다 달라진다 — 수집자가 화면을 확인하려 몸을 기울이고, 회전
구간에서 위치가 바뀌고, 턱에 걸리며 흔들린다. 정적 이미지 마스크는 그 변동 앞에서 두 방향으로
틀린다: 사람이 없는 자리를 가리고(데이터 손실), 있는 자리를 놓친다(**틀린 라벨**). 반면 사람의
**물리적 위치**는 body 프레임에서 늘 같은 영역이다 — 이미지 어디에 찍혔든 카트 뒤 0.6m 다.
그래서 body 프레임 박스로 덮는다.

**박스 하나가 두 문제를 덮는다.**

1. *카메라 자기 가림*: 실측(지면 z=−0.87)에서 handle+human 이미지 마스크가 죽이는 셀은
   143개(0.99%), 범위 x −1.98~−0.98m·|y| 최대 0.68m(p90 0.58), 전방(x>0)은 0개다.
   기본 박스가 이 143셀을 **100% 포함**한다.
2. *맵 occupancy 오염*: `map_clean.pcd` 후방에 수집자 잔재가 `min_pts=3` 기준 12~185셀
   남아 허위 obstacle 이 된다(rawos1 최악). 이건 이미지 마스크로는 못 고친다 — visibility 가
   아니라 obstacle 쪽 오류다. 기본 박스는 Point-LIO self mask 박스(x −1.5~−0.45, |y|<0.35)를
   완전히 담으므로 이 영역도 unseen 이 된다.

**실측 대조**: 손잡이 폭 46cm(→|y|≤0.23), 수집자는 LiDAR 기준 약 60cm 후방에서 카트를 민다.
기본 박스의 `yh=0.7` 은 그보다 넉넉한데, 기울기·회전으로 실제 가림이 |y| 0.68m 까지 관측되기
때문이다.

**비용**: 896셀(6.2%). 후방 절반을 통째로 버리는 대안(4800셀, 33.3%)의 5분의 1 이다.

격자 기하(XF·XR)는 그대로 두고 박스 안에도 LiDAR occupancy 는 계속 기록된다 — visibility 만
0 이다. 남는 occupancy 오염은 최종 BEV 라벨 검수 단계에서 사람이 보정한다.
...
--self-mask-classes table          # 이미지 마스크에서 읽을 클래스(쉼표 구분)
--self-box-near 0.4                # 후방 self 박스 = -far <= x <= -near, |y| <= yh
--self-box-far 2.1                 #   기본 박스 = 896셀(6.2%)
--self-box-yh 0.7
